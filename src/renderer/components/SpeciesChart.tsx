...
export const SpeciesChart: React.FC<SpeciesChartProps> = ({ agents, width = 400, height = 250 }) => {
  ...
  useEffect(() => {
    ...
    const speciesData: SpeciesData[] = Array.from(speciesMap.entries()).map(([id, speciesAgents]) => {
    ...
    speciesData.slice(0, 5).forEach((species, i) => {
      ...
      ctx.fillText(`Fit: ${species.avgFitness.toFixed(1)}`, 120 + barWidthFit + 5, y + barHeight - 2)
    })
