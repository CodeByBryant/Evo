...
class Agent {
  ...
  private inheritTraits(parentTraits: GeneticTraits, mateTraits?: GeneticTraits): GeneticTraits {
    ...
    const baseTraits = mateTraits
      ? {
          ...
          reproductionThreshold: blend(
            ...
            mateTraits.reproductionThreshold
          ),
    ...
    const mutateGene = (traitKey: keyof GeneticTraits): void => {
      ...
      if (
        traitKey === 'sensorRayCount' ||
        traitKey === 'offspringCount' ||
        traitKey === 'memoryNeurons' ||
        traitKey === 'bodyShape'
      ) {
  ...
  public update(agents: Agent[], food: Food[], canvasWidth?: number, canvasHeight?: number): void {
    ...
    const agentOffsets = this.Sensor.agentOutput.map((e) =>
      e == null ? 0 : (1 - e.offset) * aggressionWeight
    )
    const foodOffsets = this.Sensor.foodOutput.map((e) =>
      e == null ? 0 : (1 - e.offset) * foodWeight
    )
    ...
    let inputs = [
      ...
      this.position.rotation / (Math.PI * 2)
    ]
    ...
    if (this.memoryState.length > 0) {
      ...
      for (let i = 0; i < this.memoryState.length; i++) {
        this.memoryState[i] =
          output[i % output.length] * memoryDecay + this.memoryState[i] * (1 - memoryDecay)
      }
    }
    ...
    this.currentSpeed =
      this.currentSpeed * drag + (targetSpeed - this.currentSpeed) * acceleration * 0.1
  ...
  private calculateEnergyCost(): number {
    ...
    const sensorCost =
      (this.geneticTraits.sensorRayCount / 7) *
      (this.geneticTraits.sensorRayLength / 180) *
      (this.geneticTraits.fieldOfView / 180)
  ...
  public canReproduce(): boolean {
    ...
    const ageProgress = this.age / Agent.maxAge
    const reproductionCutoff = lifeConfig.ReproductionCutoffPct || 0.9
  ...
  private getSpeciesVisualTraits(): {
    ...
    const hue = Number.isFinite(this.geneticTraits.hue)
      ? Math.floor(this.geneticTraits.hue) % 360
      : safeHash % 360
  ...
  private renderSpeciesPattern(
    context: CanvasRenderingContext2D,
    traits: ReturnType<typeof this.getSpeciesVisualTraits>
  ): void {
  ...
  public render(context: CanvasRenderingContext2D, isSelected: boolean = false): void {
    ...
    context.fillStyle = isSelected
      ? '#ffff00'
      : `hsl(${traits.hue}, ${traits.saturation}%, ${traits.lightness}%)`
    context.fill()
    context.strokeStyle = isSelected
      ? '#ffaa00'
      : renderConfig.StrokeColor ||
        `hsl(${traits.hue}, ${traits.saturation}%, ${traits.lightness - 15}%)`
